...
>
>We recommend using the [`exhaustive-deps`](https://github.com/facebook/react/issues/14920) rule as part of our [`eslint-plugin-react-hooks`](https://www.npmjs.com/package/eslint-plugin-react-hooks#installation) package. It will ensure that components handle updates consistently. To make sure that all dependencies are specified correctly, it statically checks all variables used within a hook. Complex expressions (such as `foo.bar[baz]`) are not able to be checked statically and will generate a warning. Omitting variables used within a hook or including variables that are not needed witll also generate warnings. See [`here`](https://github.com/facebook/react/issues/14920#issuecomment-471070149) for a more in-depth explanation for why these things are considered harmful.
